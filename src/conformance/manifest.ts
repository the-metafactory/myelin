 ...
 *                (strict equality, default ceiling, §6.0 partner check). (11)
 *   myelin#11  — RFC-0005 sovereignty engine debt (kebab NAK tokens, §8 nak
 *                source grammar). (2)
 *
...
export const MANIFEST: Record<string, ManifestEntry> = {
  ...
  // ── identifiers ──
  "bind/agent-prefix-accept": { issue: "myelin#238", note: "no agent-prefix anti-impersonation checker on main" },
  "bind/agent-prefix-reject": { issue: "myelin#238", note: "no agent-prefix anti-impersonation checker on main" },
  "decode/agent-roundtrip-back": { issue: "myelin#238", note: "no injective decodeDidSegment on main; lands with the ./wire codec" },
  "inv/agent-arity-short": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/did-url-fragment": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/did-url-path": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/did-url-query": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/double-hyphen": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/empty-segment": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/leading-digit": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/legacy-classless": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/principal-arity-long": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/resolve-self-asserted": { issue: "myelin#238", note: "registry resolve() is a bare map lookup with no resolution-plane / self-asserted refusal" },
  "inv/segment-too-long": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/trailing-hyphen": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/underscore": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/unknown-tag": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/uppercase": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "inv/wallet-reserved-not-a-tag": { issue: "myelin#238", note: "fail-closed class+arity parseDid with reason tokens is unbuilt; DID_RE is a bare pre-cut regex (no token, and accepts what RFC-0001 rejects)" },
  "stack-id/literal-default-slug-masking": { issue: "myelin#238", note: "no {principal}/{stack} parser (no-default-fabrication) on main" },
  "stack-id/non-default-slug": { issue: "myelin#238", note: "no {principal}/{stack} parser (no-default-fabrication) on main" },
  ...
  // ── sovereignty ──
  "egress/block-local-escape": { issue: "myelin#261", note: "egress procedure: kebab compliance_block tokens + residency/classification engine gaps" },
  "egress/local-to-federated-block": { issue: "myelin#261", note: "egress procedure: kebab compliance_block tokens + residency/classification engine gaps" },
  "egress/public-to-local-block": { issue: "myelin#261", note: "egress procedure: kebab compliance_block tokens + residency/classification engine gaps" },
  "egress/residency-listed-mismatch-block": { issue: "myelin#261", note: "egress procedure: kebab compliance_block tokens + residency/classification engine gaps" },
  "ingress/chain-earlier-stamp-unmapped-invalid": { issue: "myelin#261", note: "ingress procedure: kebab reason tokens + engine gaps (permissive-ALLOW, partner-unknown dead value, chain-walk gated off, scope ceiling)" },
  "ingress/mapped-capability-exceeds-ceiling-block": { issue: "myelin#261", note: "ingress procedure: kebab reason tokens + engine gaps (permissive-ALLOW, partner-unknown dead value, chain-walk gated off, scope ceiling)" },
  "ingress/mapped-subject-outside-scope-block": { issue: "myelin#261", note: "ingress procedure: kebab reason tokens + engine gaps (permissive-ALLOW, partner-unknown dead value, chain-walk gated off, scope ceiling)" },
  "ingress/partner-unknown-link-rejected": { issue: "myelin#261", note: "ingress procedure: kebab reason tokens + engine gaps (permissive-ALLOW, partner-unknown dead value, chain-walk gated off, scope ceiling)" },
  "ingress/unknown-principal-permissive-ceiling-block": { issue: "myelin#261", note: "ingress procedure: kebab reason tokens + engine gaps (permissive-ALLOW, partner-unknown dead value, chain-walk gated off, scope ceiling)" },
  "ingress/unknown-principal-reject": { issue: "myelin#261", note: "ingress procedure: kebab reason tokens + engine gaps (permissive-ALLOW, partner-unknown dead value, chain-walk gated off, scope ceiling)" },
  "ingress/unsigned-block": { issue: "myelin#261", note: "ingress procedure: kebab reason tokens + engine gaps (permissive-ALLOW, partner-unknown dead value, chain-walk gated off, scope ceiling)" },
  ...
  "decode/agent-dotform-subject": { issue: "myelin#238", note: "no injective decodeDidSegment on main; lands with the ./wire codec" },
