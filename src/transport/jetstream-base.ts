...
/**
 * Options shared by every JetStream-backed transport, independent of how
 * the underlying NATS connection is established (TCP vs WebSocket).
 */
 ...
 * by `getConsumerHealth`. All counts are absolute (cumulative) —
 * sample twice and subtract to compute throughput between observation
 * windows. `pending` and `ackPending` are the key early-warning
 * signals for a stuck consumer.
 *
...
/**
 * Operator-facing knobs for `ensureStream`. Each field maps to the
 * matching `jsm.streams.add` argument; defaults reflect what
...
/**
 * Shared JetStream transport machinery — everything a myelin transport
 * does AFTER a `NatsConnection` exists: envelope codec handling,
 * JetStream publish, request/reply over core-NATS inboxes, durable +
 * best-effort subscriptions, consumer health, and stream management.
 *
 * Concrete subclasses differ ONLY in how the connection is established
 * (`establishConnection`): `NATSTransport` dials raw TCP via
 * `@nats-io/transport-node` (Node/Bun), `WebSocketTransport` dials
 * `wsconnect` (browsers, Cloudflare Workers/Durable Objects, Deno).
 *
 * Portability rule: this file MUST NOT reference Node-only globals
...
export abstract class BaseJetStreamTransport implements TransportPublisher, TransportSubscriber {
  ...
  private readonly baseOptions: JetStreamTransportOptions;
  ...
  /**
   * Establish the underlying NATS connection. Called lazily on first
   * use; the result is cached until `close()`. Implementations own
   * authentication wiring and MUST throw (not warn) when their
   * `requireAuth` contract is violated.
   */
   ...
   * `process.stderr.write` byte-exact behavior.
   */
  protected logError(message: string): void {
    console.error(message);
  }

  protected decodeEnvelope(data: Uint8Array): MyelinEnvelope {
    const detected = detectCodec(data) ?? this.codec.id;
    return this.codecRegistry.get(detected).decode(data);
  }

  protected async ensureNc(): Promise<NatsConnection> {
    if (this.nc) return this.nc;
    this.nc = await this.establishConnection();
    return this.nc;
  }
