...
export function selectWinner(
  ...
  switch (strategy) {
    ...
    case "lowest-cost": {
      const withCost = eligible.filter((b) => typeof b.cost === "number");
      if (withCost.length === 0) return null;
      let best = withCost[0]!;
      for (let i = 1; i < withCost.length; i++) {
        if ((withCost[i]!.cost as number) < (best.cost as number)) best = withCost[i]!;
      }
      return { winner: best, reason: `lowest-cost: ${(best.cost as number).toFixed(4)}` };
    }
    case "highest-match": {
      let best = eligible[0]!;
      for (let i = 1; i < eligible.length; i++) {
