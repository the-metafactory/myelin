...
> **Scope:** This document defines the Myelin layer model — the contracts each layer guarantees, the code that implements them today, and the issues that track open work.
>
...
Myelin is the layer model the metafactory ecosystem runs on — the envelopes, transports, identities, and composition patterns that connect agents across networks. It is a set of layers, not a single thing.

The discipline of the OSI / TCP-IP layered model — narrow inter-layer interfaces, swappable implementations, explicit cross-layer concerns — is what made the internet's layered protocols durable across forty years of underlying-tech turnover. Applying that lens to myelin while it is still small costs less than retrofitting it later.

The model below is **the canonical Myelin layer model**. It supersedes the v4 nervous-system five-layer naming (MYELIN / AXON / DENDRITE / SYNAPSE / CORTEX). Two changes from v4: Identity is named as its own layer, and CORTEX is repositioned as an M7 application rather than a peer layer. Rationale in [myelin#5](https://github.com/the-metafactory/myelin/issues/5).

## 2. The Myelin layer model
...
├─────────────────────────────────────────────────────────────┤
│  M4  IDENTITY        Verifiable principal · signed_by       │
│                      chain · sovereignty attestation        │
├─────────────────────────────────────────────────────────────┤
│  M3  ENVELOPE        Envelope schema · canonical form ·     │
│                      sovereignty metadata · namespace       │
├─────────────────────────────────────────────────────────────┤
│  M2  TRANSPORT       Bus · pub/sub · request/reply ·        │
│                      delivery guarantees · subjects         │
├─────────────────────────────────────────────────────────────┤
│  M1  CONNECTIVITY    TCP · TLS · NATS leaf-node topology    │
...
flowchart TB
  ...
  M3["M3 ENVELOPE<br/>schema · canonical form · sovereignty"]
  M2["M2 TRANSPORT<br/>bus · pub/sub · subjects"]
  M1["M1 CONNECTIVITY<br/>TCP · TLS · NATS topology (out of scope)"]
...
|---|---|---|---|---|
| **M7 Surfaces** | Applications consuming the layers below | (other repos: cortex, pilot, signal) | — | external |
| **M6 Composition** | Patterns for combining envelopes (pipeline, fan-out, request/reply, negotiation) | `src/composition/`, `src/bidding/` | [#10](https://github.com/the-metafactory/myelin/issues/10) | partially implemented (orchestrator, workflow schemas in `src/composition/`; bidding/negotiation in `src/bidding/`; spec #10 still open) |
| **M5 Discovery** | Runtime queryable capability registry | `src/discovery/` | [#9](https://github.com/the-metafactory/myelin/issues/9) | implemented (signed self-advertisements; NATS capability store deferred) |
| **M4 Identity** | Verifiable per-envelope identity; signed_by chain | `src/identity/`, `src/agent-identity/` | [#8](https://github.com/the-metafactory/myelin/issues/8) (closed), [#31](https://github.com/the-metafactory/myelin/issues/31) (chain) | implemented (chain-of-stamps shipped #31, PR #92) |
| **M3 Envelope** | Envelope schema, canonical encoding, sovereignty metadata, NATS namespace | `src/envelope.ts`, `src/types.ts`, `schemas/envelope.schema.json`, `specs/namespace.md` | [#6](https://github.com/the-metafactory/myelin/issues/6) (namespace) | implemented |
| **M2 Transport** | Abstract bus interface; pub/sub + request/reply; subject-based addressing | `src/transport/` | [#12](https://github.com/the-metafactory/myelin/issues/12) (closed) | implemented (NATS + InMemory) |
| **M1 Connectivity** | TCP, TLS, NATS leaf-node topology | (NATS server config; not in this repo) | — | out of scope |

**Cross-layer:** [myelin#11](https://github.com/the-metafactory/myelin/issues/11) — sovereignty enforcement protocol that cuts across M3 (declared), M4 (attested), and M2 (enforced).
...
**Why we name it.** OSI taught us that pretending the lower layer doesn't exist leads to buggy higher layers. Myelin assumes M1 provides authenticated, encrypted, ordered byte streams. If M1 fails (network partition, TLS expiry), every layer above degrades together — that's a feature, not a bug, and the layer model surfaces the dependency cleanly.
...
- `src/subjects.ts` + `src/subject-matching.ts` + `src/subject-vocabulary.ts` + `src/segment-validators.ts` + `src/patterns.ts` + `src/classifications.ts` — the subject namespace grammar (derivation, matching, segment validation, classification vocabulary) shared by M2 addressing and M3 envelope routing.
...
- `src/dispatch/` — task-dispatch subjects, correlation, and payload-identity plumbing (M3 envelope ↔ M6 routing).
- `src/lifecycle/` — task lifecycle event types and emission (M3 payload shapes consumed at M6).

**Source-of-truth issue.** [myelin#6](https://github.com/the-metafactory/myelin/issues/6) (MY-101 namespace).

**Status.** Implemented. This is the cleanest layer in the model — designed to a contract from the start, no transport coupling.

**Inside vs outside the signature.** The envelope distinguishes attested fields (inside signature) from mutable fields (`correlation_id`, `economics`, `extensions`). This is a load-bearing M3 invariant; the trust contract that follows from it (clients MUST NOT make trust decisions based on mutable values) is stated in §5.2.
...
- `src/agent-identity/` — key generation, rotation, encryption, and per-agent identity storage that feed the M4 signing keys.

**Source-of-truth issues.**
- [myelin#8](https://github.com/the-metafactory/myelin/issues/8) (closed) — original M4 identity spec.
...
**Cross-layer notes.** M4 attests origin, and *path* whenever each hop appends its own stamp: the chain is the mechanism that makes per-hop M6 sovereignty enforcement possible, not just first-hop trust. (Whether every hop actually appends a stamp is a deployment choice; the capability is what M4 provides.)
...
- `advertisement-identity.ts` + `canonicalize.ts` — sign/verify advertisements over canonical bytes (reuses the M4 signing discipline).
...
### M6 — Composition *(partially implemented)*
...
**Status.** Partially implemented. The orchestrator (pipeline / fan-out / fan-in / dependency-ordered execution with recovery) and the bidding/negotiation subsystem are in-tree and tested; the overarching M6 composition spec (#10) is still open, so the canonical pattern catalogue is not yet frozen.
...
Not every module belongs to a single layer. These sit under the layer model and are consumed across it:
...
- `src/wire/generated/` — **generated** wire artifacts ([#237](https://github.com/the-metafactory/myelin/issues/237)): anchored regexes, JSON-Schema `pattern` fragments, and TS `const` enums derived by `tools/abnf-gen` from the normative `specs/grammar/*.abnf` grammars. Never hand-edited — `bun tools/abnf-gen` regenerates, and CI's `bun tools/abnf-gen --check` fails on drift. The ratified (flag-day-R) forms are staged under `src/wire/generated/r/` per [`docs/design-rfc-alignment.md`](./design-rfc-alignment.md) D6, until the cut swaps them into `src/wire/generated/`.
...
The closed **residency registry** (`src/residency-registry.ts`, RFC-0005 §2.3; grill D5) backs the residency dimension: `sovereignty.data_residency` is validated at M3 envelope validation against the assigned ISO 3166-1 alpha-2 set plus the `EU` convention and is fail-closed, so an unassigned code can never reach the M2 egress `data_residency_constraints` lookup and dodge residency gating. It is a leaf module (no imports), the same containment pattern as `src/classifications.ts`.

### 5.2 Mutable fields are NOT trust-bearing

`correlation_id`, `economics`, and `extensions` are intentionally outside the M4 signature so intermediaries can annotate observability and economics state without invalidating attestations. **Hard contract:** clients MUST NOT make security or trust decisions based on mutable-field values. Anything that needs to be both mutable AND attested is a signal to add a new attested mechanism (e.g. a per-stamp extensions bag on the shipped chain-of-stamps), not to expand the carve-out.

### 5.3 Transport-independence

M4 identity verification MUST work regardless of which M2 transport delivered the envelope. The signature covers envelope content, not transport metadata. A bot's identity is the same whether the message rode NATS, an HTTP webhook bridge, or anything else.

### 5.4 Network sovereignty over registries

Each network owns its identity registry (M4) and its capability registry (M5). There is no global authority. Cross-network trust is established by explicit federation handshake (out of scope for v1).

### 5.5 Observability (cross-layer)

`src/observability/` provides a transport-wrapping metrics layer (`transport.ts`, `histogram.ts`) that records publish/subscribe/latency signals without any layer above having to instrument itself. It is deliberately cross-layer: it observes M2 traffic but is consumed by deployment tooling and M7 surfaces. Like sovereignty enforcement, it wraps the transport rather than living inside it.

## 6. Design conventions

These are repo-wide conventions that follow from the layered model:

- **No layer skipping in code.** Higher-layer code does not import lower-layer concrete implementations directly. M7 code that needs to publish does so through M3+M4 (envelope + signed_by) over M2 (abstract transport), never by speaking NATS directly.
...
- **JCS** — JSON Canonicalization Scheme, RFC 8785. Used at M4 to produce deterministic signing bytes.
...
|---|---|
| M1 | external |
| M2 | implemented (NATS + WebSocket + InMemory) |
| M3 | implemented |
| M4 | implemented (chain-of-stamps shipped, #31/PR #92) |
| M5 | implemented (signed self-advertisements, #9); NATS capability store deferred |
| M6 | partially implemented (orchestrator + bidding in-tree; spec #10 open) |
| M7 | external (per-repo) |
