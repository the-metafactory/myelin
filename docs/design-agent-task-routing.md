...
- *M7 attestation worked example* — a utility's internal AI-agent governance standard (twelve normative requirements) covering: scoped service principals, prod-write prohibition, egress allow-listing, sandbox-by-default, tool/MCP supply-chain pinning, sub-agent trust floor, append-only session logs with threshold review, and similar concerns. The standard itself is a deploying-organisation-internal artefact; it is cited here as a concrete instance of the *kind* of attestation surface M7 must host. The architectural argument does not depend on any reader being able to access the standard.
...
8. **Implement nak-based sovereignty evaluation** — agent-side task filter using its M7 deployment policy (cortex.yaml `agents[].roles / .trust` and any deploying-organisation attestation slots)
...
**Out of scope for this protocol** (M7 concerns — see §Stratification): orchestrator translation logic, agent compliance attestation surface (the deploying organisation's internal AI-agent governance standard), notification surface routing (cortex surface-router), threshold-review for slow-motion harm (consumes lifecycle stream from M7).
