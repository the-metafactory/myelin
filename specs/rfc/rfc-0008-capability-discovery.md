...
; capability-id — the CONVERGED canonical grammar (RFC §4.1, grill D1):
; one or more "."-separated segments, EACH a capability-tag (the task-side
; kebab rule). A single-segment id is therefore backward-compatible with
; every existing tag; the id's dots project 1:1 onto NATS subject segment
; boundaries (tasks.{capability}.{subcapability}, ratified RFC-0002).
; Matching is SEGMENT-PREFIX (RFC §4.2). Underscore ids migrate at flag-day
; R (kebab-strict forbids "_" — ratified RFC-0001).
capability-id           = capability-tag *( "." capability-tag )
...
; capability-id-compound — the RETIRED wider cortex regex, recorded for
; migration only (CAPABILITY_ID_REGEX, cortex src/common/types/capability.ts:172).
; Post-D1 it is a superset alias: ids also valid under capability-id are
; canonical; ids exercising the wider alphabet ("_", consecutive/trailing
; hyphens — e.g. "federated.subject_dispatch") are NON-CONFORMANT and
; migrate at flag-day R. The regex is a named conformance defect to tighten.
