...
- [RFC-0003] metafactory, "Envelope", **Ratified**. Owns the envelope fields (`source`, `type`, `target_assistant`, `distribution_mode`, `sovereignty.classification`, `requirements`) that §8's derivation consumes. Co-files source authority (signed-wins, D6/D7) with this document; the signed `source` representation governs the stack (§8.1).
...
- [RFC-0008] metafactory, "Capability Discovery and Advertisement", **Ratified**. Normative owner of the `capability-id` grammar and the capability-set reconciliation referenced in §8.5. Per D15 it MUST constrain the `capability-id` charset to be subject-safe (charset ∩ `segment`) so the projection into the `tasks` slot is 1:1.
...
- [RFC-0005] metafactory, "Sovereignty and Boundary Crossing", **Ratified**. Its sovereignty enforcement-NAKs **fold under `_audit.sovereignty.*`** — no separate `_nak.` prefix (§9, §10, D21).
- [RFC-0007] metafactory, "Transport and Reliability", **Ratified**. Owns the `_INBOX.` grammar, **admitted by reference** into the reserved-prefix table (§9, §10, D22); owns the `TASKS_DEAD` stream-filter alignment side of the legacy-form retirement (§8.2).
