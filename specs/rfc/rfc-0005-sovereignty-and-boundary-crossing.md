...
> **Actor-authority cap on the self-asserted plane (RFC-0003 §7, myelin#251).** Ingress keys on the
> last-stamp signer principal (§6.1); the **actor** an admitted envelope asserts is a separate
> resolution (RFC-0003 §7 — `originator.identity` when present). RFC-0003 §7 adds a normative cap
> this procedure composes with: when the resolved actor is a `surface`-, `system`-, or `hub`-class
> DID, the ingress authorization MUST NOT grant it **principal-scoped** authority (capabilities,
> sovereignty, or trust belonging to a principal). Those classes carry no principal component and
> are not bound to the signing chain by the RFC-0003 §3.17 reconciliation (which fires only for
> `principal`/`stack`-class originators), so authority read from one is authority any keyed signer
> can mint. A self-asserted or hub actor is authorized only within its own non-principal scope; the
> scope ceiling of §6.3 still applies to the signer.
