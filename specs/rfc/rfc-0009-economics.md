...
| 2026-07-17 | Ratified | **Editorial (no wire change): Appendix B.2 reject fixtures extracted to a runnable file** (myelin#252). The §6 conformance clause (:496–499) already made `invalid.json` a MUST-reject input, and B.2 already authored the reject fixtures — but no `specs/vectors/economics/invalid.json` existed in the tree (the myelin#236 item-25 "noted exception … not corrected", recorded before the §6 dangling reference was noticed). All **8** authored B.2 rejects are now extracted verbatim to that file (the issue named 5; B.2 authors 8 — the extraction captures the full set so §6's "every invalid.json input" and the acceptance's "ids match Appendix B.2" both hold). The `billing_ref` 257-char placeholder is materialised as a literal 257-char value (matching the valid.json boundary-vector convention). `specs/vectors/README.md` exception note updated to "resolved". No normative/wire content changed; the RFC body and Appendix B.2 are untouched. Fix prompted by external review (NorthwoodsSentinel, PR #230). |
