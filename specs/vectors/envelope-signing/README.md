...
| `reject.json` | `validateStampSyntax`, `verifyEnvelopeIdentity` | The §11 rejection-token matrix (D27); D8 verification-equation edge cases; D6 chain-length cap; D16 stackless fail-closed; the non-agent originator binding REJECT half (myelin#251 — `originator-principal-binding-violation`: cross-principal principal/stack, the federated-forward s[0]-anchor case, and the hub-class-innermost-signer fail-closed case) |
